...
ap = argparse.ArgumentParser(
    description="Two-stage outcome pseudo-labelling (PT-mode fill, then overall-mode fill).")
ap.add_argument("--data", default=os.environ.get("FAERS_ML_DATASET", "FAERS_Pediatric_ML_Dataset.xlsx"),
                help="Path to FAERS_Pediatric_ML_Dataset.xlsx. This file is NOT distributed "
                     "with the repository; supply your own copy via --data or the "
                     "FAERS_ML_DATASET environment variable.")
args = ap.parse_args()

if not os.path.exists(args.data):
    sys.exit(f"Input dataset not found: {args.data}\n"
             "FAERS_Pediatric_ML_Dataset.xlsx is not shipped with this repository. "
             "Rebuild it from the FAERS quarterly ASCII files using the pipeline "
             "specification in S4 Table, then pass it with --data.")
